...
from domain.entities.user import UserInDB, UserCreate
...
from application.factories.service_factory import get_user_service, get_category_service
...
class UserOrchestrator:
    ...
    # async def get_user_categories(self, user: UserIdDTO) -> list[CategoryInDb]:
    #     category_service = await get_category_service(self.uow.category_repo)
    #     user_id_entity = await self.user_id_mapper.dto_to_entity(user)
    #     user_categories = await category_service.get_user_categories(user.user_id)
    #     user_categories_dto = await self.
