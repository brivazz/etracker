...
from interfaces.telegram_bot.handlers.orchestrators.add_orchestrator import (
    AddOrchestrator,
)
from interfaces.telegram_bot.handlers.orchestrators.category_orchestrator import (
    CategoryOrchestrator,
)
from interfaces.telegram_bot.handlers.orchestrators.stats_orchestrator import (
    StatsOrchestrator,
)
from interfaces.telegram_bot.handlers.orchestrators.common_orchestrator import (
    CommonOrchestrator,
)
...
class BotService:
    ...
    async def handle_messages(self, event: events.NewMessage.Event, user: UserInDBDTO):
        ...
        logger.info(
            f"в состоянии: == {current_state} == на всякий raw_text: {event.raw_text}"
        )
        if handler is None:
    ...
    def register_handlers(self):
        ...
        async def callback_handler(
            event: events.CallbackQuery.Event, user: UserInDBDTO
        ):
