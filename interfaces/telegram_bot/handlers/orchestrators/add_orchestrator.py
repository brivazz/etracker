...
from interfaces.telegram_bot.keyboards.build_expense_keyboard import (
    ...
    edit_last_expense_keyboard,
    after_repeat_last_expense_keyboard,
    after_delete_last_expense_keyboard,
    after_input_amount_for_edit_expense_keyboard,
    expense_keyboard,
...
class AddOrchestrator(OrchestratorBase):
    ...
    async def _parse_amount(
        self, event: events.NewMessage.Event, input_message_id: int
    ) -> float | None:
        try:
            ...
            await show_error_keyboard(
                event,
                text="❌ Введите корректную сумму (например, 123.45).",
    ...
    async def save_new_expense(
        ...
        dto = ExpenseCreateDTO(
            user_id=user.id, amount=amount, category_id=category_id, note=""
        )
    ...
    async def handle_edit_expense(
        ...
        if new_amount is None:
            return
        dto = ExpenseEditDTO(
            id=meta.expense_id, amount=new_amount, category_id=meta.category_id, note=""
        )
        expense = cast(ExpenseInDBDTO, await self._handle(Command.EDIT_EXPENSE, dto))

        await event.message.delete()
        await after_input_amount_for_edit_expense_keyboard(
            event, new_amount, meta.message_id, meta.category_name
        )
        await self.fsm.set_state(
            user.telegram_id,
            State.EXPENSE_RECORDED,
    ...
    async def handle_repeat_last_expense(
        ...
        if not last_expense:
            await show_error_keyboard(
                event, text="❗ Нет последней траты для повторения."
            )
            return
        ...
        await after_repeat_last_expense_keyboard(
            event, expense.amount, last_expense.category_name
        )
        await self.fsm.set_state(
            user.telegram_id,
    ...
    async def handle_delete_last_expense(
        ...
        if not last_expense:
            await show_error_keyboard(
                event, text="❗ Нет последней траты для удаления."
            )
            return
        ...
        await after_delete_last_expense_keyboard(
            event, last_expense_amount, last_expense_category_name
        )
