...
from application.dto.user_dto import UserIdDTO
...
async def expense_keyboard(user_id: int):
    ...
    async with get_uow() as uow:
        ...
        categories = cast(
            list[CategoryInDBDTO],
            await main_orchestrator.handle_command(
                command=Command.GET_USER_CATEGORIES, dto=dto
            ),
        )
...
async def before_input_amount_keyboard(category_name: str):
    ...
    new_text = (
        f"Вы выбрали категорию: **{category_name.upper()}**. Введите сумму траты:"
    )
    new_buttons = await default_nav_buttons_keyboard()
    return new_text, new_buttons


async def before_save_amount_keyboard(
    ...
    buttons = [
        [
            Button.inline("✏️ Изменить", b"change_expense"),
            Button.inline("❌️ Отменить", b"cancel"),
        ],
...
async def after_input_amount_for_edit_expense_keyboard(
    event: events.NewMessage.Event,
    new_amount: float,
    input_message_id,
    category_name: str,
...
async def after_repeat_last_expense_keyboard(
    event: events.CallbackQuery.Event, last_expense_amount: float, category_name: str
):
...
async def after_delete_last_expense_keyboard(
    event: events.CallbackQuery.Event, last_expense_amount, category_name
):
