...
async def show_error_keyboard(event, text: str, input_message_id: int | None = None):
    ...
    try:
        if input_message_id:
            ...
            await event.respond(f"❗ {text}", buttons=await default_nav_buttons_keyboard())
        else:
...
async def main_menu_keyboard():
    ...
    return [
        ...
        [Button.inline("📊 Статистика", b"stats_expense"), Button.inline("📄 История трат", b"history")],
        [Button.inline("🔁 Повторить последнюю", b"repeat_expense"), Button.inline("✏️ Редактировать последнюю", b"edit_expense")],
