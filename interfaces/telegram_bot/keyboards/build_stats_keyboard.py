...
async def expense_history_keyboard_keyboard(event: events.CallbackQuery.Event):
    ...
    buttons = [
        [
            Button.inline("📅 По дням", b"history:day"),
            Button.inline("🗓️ По неделям", b"history:week"),
        ],
        [
            Button.inline("📆 По месяцам", b"history:month"),
            Button.inline("🗃️ По годам", b"history:year"),
        ],
...
async def after_expense_history_period_keyboard(
    event: events.CallbackQuery.Event, text: list[str]
):
