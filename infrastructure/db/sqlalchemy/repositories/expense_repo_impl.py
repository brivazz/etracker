...
class SQLAlchemyExpenseRepository(AbstractExpenseRepository):
    ...
    async def get_user_expenses(self, user_id: int) -> list[ExpenseInDB]:
        result = await self.session.execute(
            select(ExpenseORM).where(ExpenseORM.user_id == user_id)
        )
    ...
    async def get_last(self) -> ExpenseInDB | None:
        stmt = (
            select(ExpenseORM, CategoryORM.name.label("category_name"))
    ...
    async def get_stats(self, user_id: int, from_date: datetime) -> list[StatsInDb]:
        stmt = (
            select(CategoryORM.name, func.sum(ExpenseORM.amount))
            .join(CategoryORM, ExpenseORM.category_id == CategoryORM.id)
            .where(ExpenseORM.user_id == user_id, ExpenseORM.created_at >= from_date)
    ...
    async def get_expense_history(
        ...
        if self.session.bind.dialect.name == "sqlite":
            if period == "week":
                ...
                trunc_date = func.strftime(
                    strftime_patterns[period], ExpenseORM.created_at
                )
        else:
